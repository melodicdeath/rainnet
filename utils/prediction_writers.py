        ...
        with h5py.File(self.db_name, "a") as db:
            ...
            for bi, b_idx in enumerate(batch_indices_):
                ...
                group_name = self.group_format.format(
                    timestamp=common_time, method=self.method_name
                )
                group = db.require_group(group_name)

                n_leadtimes = (
                    prediction.shape[1]
                    if prediction.ndim == 4  # B, T, W, H
                ...
                for i in range(n_leadtimes):
                    ...
                    what_attrs["validtime"] = np.bytes_(f"{date:%Y-%m-%d %H:%M:%S}")
                    packed = arr_compress_uint8(
                        data_unpacked.detach().cpu().numpy(), missing_val=0
                    )
                    write_image(
                        group=ds_group,
                        ds_name="data",
