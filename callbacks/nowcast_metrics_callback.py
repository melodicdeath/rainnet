...
class NowcastMetrics(Callback):
    """
    Callback to calculate deterministic nowcast skill metrics for the model attached,
    and save those metrics and their plots to the model logger / external files.
    """
    ...
    def __init__(self, config) -> None:
        ...
        if config.input_mode == "deterministic":
            self.input_mode = config.input_mode
        elif config.input_mode == "probabilistic":
            self.input_mode = config.input_mode
        else:
            raise ValueError(
                f"Input mode {config.input_mode} \
                undefined.\n Choose 'deterministic'\ or 'probabilistic'."
            )

    def setup(
        self, trainer: "pl.Trainer", pl_module: "pl.LightningModule", stage=None
    ) -> None:
        ...
        self.valid_scores.add_metrics(
            {
                f"ETS_{thr}": ETS(
                    threshold=thr,
                    length=pl_module.verif_leadtimes,
        ...
        self.valid_scores.add_metrics(
            {
                "MAE": MAE(
                    length=pl_module.verif_leadtimes, reduce_dims=self.reduce_dims
                ).to(dev)
            }
        )
    ...
    def on_validation_batch_end(
        ...
        _, y, _ = batch
        y = y[:, : pl_module.verif_leadtimes]
        y_hat = outputs["prediction"]
        if self.input_mode == "probabilistic":
            y_hat = torch.mean(y_hat, dim=0)  # avg samples
        if hasattr(pl_module, "mode"):
            if pl_module.mode == "heteroscedastic_regression":
                y_hat = y_hat[..., 0]  # take only pred, not variance
        to_mmh = trainer.datamodule.valid_dataset.inverse_scaler
    ...
    def on_test_batch_end(
        ...
        if hasattr(pl_module, "mode"):
            if pl_module.mode == "heteroscedastic_regression":
                y_hat = y_hat[..., 0]  # take only pred, not variance
        to_mmh = trainer.datamodule.test_dataset.inverse_scaler
    ...
    def _epoch_end(self, stage, trainer, pl_module):
        ...
        for il in range(pl_module.verif_leadtimes):
            ...
            # Continuous scores
            if pl_module.verif_leadtimes > 1:
                cont_scores[lt] = {"MAE": scores[f"MAE"][il].detach().cpu().numpy()}
            else:
        ...
        fig = verification_score_plots.plot_cat_scores_against_leadtime(
            ...
            max_leadtime=5 * (pl_module.verif_leadtimes + 1),
            write_fig=self.save_metrics,
        )
